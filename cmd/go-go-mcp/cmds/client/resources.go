...
func (c *ListResourcesCommand) RunIntoGlazeProcessor(
	...
	res, err := client.ListResources(ctx, nil)
	if err != nil {
		return err
	}
...
func (c *ReadResourceCommand) RunIntoWriter(
	...
	res, err := client.ReadResource(ctx, &mcp.ReadResourceParams{URI: s.URI})
	if err != nil {
		return err
	}
...
func (c *ReadResourceCommand) RunIntoGlazeProcessor(
	...
	for _, content := range res.Contents {
		if err := gp.AddRow(ctx, types.NewRow(types.MRP("uri", content.URI), types.MRP("mime", content.MIMEType), types.MRP("text", content.Text), types.MRP("blob", content.Blob))); err != nil {
			return err
		}
	}
	return nil
